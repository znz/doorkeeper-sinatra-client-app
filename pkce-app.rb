...
get '/login' do
  ...
  code_challenge = [Digest('SHA256').digest(session[:code_verifier])].pack('m0').tr('+/', '-_').delete('=')
  redirect client.auth_code.authorize_url(redirect_uri: redirect_uri, state: session[:state], scope: oauth2_scope, code_challenge: code_challenge, code_challenge_method: 'S256')
end
...
get '/callback' do
  ...
  client = OAuth2::Client.new(client_id, client_secret, oauth2_options)
  token = client.auth_code.get_token(params[:code], redirect_uri: redirect_uri, code_verifier: session[:code_verifier])
